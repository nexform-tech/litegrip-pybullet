...
| 样例 02（仿真 → 真机） | ⚠️ **部分验证** | 在 `can0` 的真实夹爪上跑过，触发了电机故障；斜坡修复有 `tests/test_example02_stream.py` 覆盖，但**修复本身还没上过真机** |
| 样例 02 的 `--status` 诊断 | ⚠️ **部分验证** | `--status` 在真机上能走到 `connect()` 并正确报出 CAN 接口不存在；读故障和清故障还没在真机上试过 |
| 样例 03（真机 → 仿真） | ⚠️ **部分验证** | 只读镜像路径在 `can0` 的真实夹爪上跑过；`--zero-gravity` 和 `--passive` 没跑过 |
| 真机运动指令 | ⚠️ **部分验证** | 早期版本的样例 02 向真机发过阶跃指令流，把电机打进了故障态；换成斜坡之后还没跑过 |
| 空闲保活（`IdleKeeper` / 03 的锁位帧） | ⚠️ **部分验证** | 有单测覆盖发帧节奏与间隔上限；**没上过真机** |

真机上「能读不能控 + 红灯闪」有两种原因，长得一样：

- **阶跃指令**（一帧就把目标发到底）：MIT 的位置项 `kp × (q目标 − q实际)`，
  kp = 100 Nm/rad，走满 1.845 rad 就是向一颗额定约 10 Nm 的电机要 ~185 Nm。电机
  随即锁进欠压/过流保护（0x9/0xA）。样例 02 现在跟 SDK 自己的 `goto_rad` 一样走
  斜坡，一帧一个 tick，单帧要求不到 1 Nm。
- **空闲太久**：`TIMEOUT` 寄存器（RID 9，本机实测 8000 ms）是 CAN 通信超时保护，
  连续这么久收不到帧就锁通信丢失故障（**0xD**，SDK 的 `describe_error` 还不认识
  这个码，样例自己给它起了名字，不再报成「未知错误」）。**只看不发的空闲期本身
  就是故障原因**——这是「仿真控真机只能读不能控」
  真正的主因。02 和 03 现在空闲时也发保持帧（目标 = 实测位置、零前馈），03 若要
  让别的程序驱动可以加 `--passive` 一帧不发。

两种故障都能用 `examples/02_sim_to_real.py --status [--clear-fault]` 诊断和清除，
不必移动电机。
