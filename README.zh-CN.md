...
阶跃指令（一帧就把目标发到底）在这里会打出故障：MIT 的位置项
`kp × (q目标 − q实际)`，kp = 100 Nm/rad，走满 1.845 rad 就是向一颗额定约 10 Nm
的电机要 ~185 Nm。电机随即锁进欠压/过流保护，之后位置照常回报、指令一律不执行。
样例 02 现在跟 SDK 自己的 `goto_rad` 一样走斜坡，一帧一个 tick，单帧要求不到 1 Nm；
故障可以用 `examples/02_sim_to_real.py --status --clear-fault` 在不移动电机的前提下清掉。
